'use strict';
/* GENERATED from src/renderer/src/scene/office/portraitArt.ts sha256:c06569e2c37b8133c513443d242e154011d367a5d816da62226c8f14ef10120b — DO NOT EDIT.
...
exports.AVATAR_RECIPES = exports.AVATAR_VOCAB = exports.SCENE_H = exports.SCENE_W = exports.PORTRAIT_H = exports.PORTRAIT_W = void 0;
exports.composeAvatar = composeAvatar;
exports.sceneFrameBufs = sceneFrameBufs;
exports.registerAvatarOverride = registerAvatarOverride;
exports.paintPortrait = paintPortrait;
exports.PORTRAIT_W = 18;
exports.PORTRAIT_H = 28;
...
let CUR_W = exports.PORTRAIT_W, CUR_H = exports.PORTRAIT_H;
...
const styleShort = (buf, color, skinBase, a) => {
    ...
    if (recede) {
        for (let y = 3; y < 6; y++)
            for (let x = 6; x < 12; x++)
                if (eq(rgbAt(buf, x, y), base))
                    set(buf, x, y, skinBase);
        set(buf, 8, 5, base); // widow's peak
    }
    const hx = part === 'L' ? 6 : 11;
    for (let y = 2; y < 6; y++)
        set(buf, hx, y, sh);
    for (let x = HX0; x < hx; x++)
        if (alphaAt(buf, x, 3))
            set(buf, x, 3, hi);
    for (let x = HX0; x <= HX1; x++)
        if (alphaAt(buf, x, 2))
...
const styleFrame = (buf, color, skinBase, a) => {
    ...
    for (let x = HX0 - 1; x < HX0 + 1; x++)
        set(buf, x, length + 1, base);
    for (let x = HX1; x < HX1 + 2; x++)
        set(buf, x, length + 1, base);
    for (let y = 2; y < 6; y++)
        if (alphaAt(buf, HX1, y))
            set(buf, HX1, y, sh);
    for (let x = HX0; x < 9; x++)
        if (alphaAt(buf, x, 2))
            set(buf, x, 2, hi);
...
const styleCurly = (buf, color, skinBase) => {
    ...
    set(buf, 8, 6, skinBase);
    set(buf, 9, 6, skinBase);
    for (const [x, y] of [[5, 2], [7, 2], [9, 2], [11, 2]])
        set(buf, x, y, hi);
};
...
function drawClothing(buf, kind, c1, c2, tie, skin, heavy = false) {
    ...
    if (kind === 'suit') {
        ...
        for (const [x, y] of [[8, 19], [9, 19], [7, 20], [8, 20], [9, 20], [10, 20], [8, 21], [9, 21]])
            set(buf, x, y, white);
        for (const [x, y] of [[6, 20], [7, 21], [11, 20], [10, 21], [6, 21], [11, 21]])
            set(buf, x, y, sh);
        if (tie) {
            for (let y = 20; y < 26; y++) {
                set(buf, 8, y, tie);
        ...
        for (const [x, y] of [[6, 19], [7, 19], [10, 19], [11, 19], [7, 20], [10, 20]])
            set(buf, x, y, sh);
        for (let y = 20; y < 27; y += 2)
            set(buf, 8, y, sh);
        if (tie)
            for (let y = 19; y < 26; y++) {
                set(buf, 8, y, tie);
        ...
        for (const [x, y] of [[6, 19], [7, 19], [10, 19], [11, 19]])
            set(buf, x, y, hi);
        set(buf, 8, 20, sh);
        set(buf, 8, 22, sh);
        const accent = c2 ? shades(c2)[1] : hi;
        for (const [x, y] of [[7, 20], [9, 20]])
            set(buf, x, y, accent);
    }
    else if (kind === 'blouse') {
        const s = SKIN[skin];
        for (const [x, y] of [[7, 19], [8, 19], [9, 19], [10, 19], [8, 20], [9, 20]])
            set(buf, x, y, s.sh);
        for (let x = 5; x < 13; x++)
            if (eq(rgbAt(buf, x, 20), base))
                set(buf, x, 20, hi);
    }
        ...
        for (const [x, y] of [[6, 19], [7, 19], [8, 19], [9, 19], [10, 19], [11, 19]])
            set(buf, x, y, sh);
    }
}
...
function drawSceneTorso(buf, r, back) {
    ...
    if (r.cloth === 'suit') {
        ...
        for (const [x, y] of [[8, 18], [9, 18], [7, 19], [8, 19], [9, 19], [10, 19], [8, 20], [9, 20]])
            set(buf, x, y, white);
        for (const [x, y] of [[6, 19], [7, 20], [11, 19], [10, 20]])
            set(buf, x, y, sh);
        if (r.tie) {
            for (let y = 19; y <= 24; y++) {
                set(buf, 8, y, r.tie);
        ...
        for (const [x, y] of [[6, 18], [7, 18], [10, 18], [11, 18], [7, 19], [10, 19]])
            set(buf, x, y, sh);
        if (r.tie)
            for (let y = 18; y <= 24; y++) {
                set(buf, 8, y, r.tie);
            ...
            for (let y = 20; y <= 24; y += 2)
                set(buf, 8, y, sh);
    }
    else if (r.cloth === 'polo') {
        for (const [x, y] of [[6, 18], [7, 18], [10, 18], [11, 18]])
            set(buf, x, y, hi);
        set(buf, 8, 19, sh);
        set(buf, 8, 21, sh);
    }
    else if (r.cloth === 'blouse') {
        for (const [x, y] of [[7, 18], [8, 18], [9, 18], [10, 18], [8, 19], [9, 19]])
            set(buf, x, y, skin.sh);
        for (let x = 5; x < 13; x++)
            if (eq(rgbAt(buf, x, 19), base))
                set(buf, x, 19, hi);
    }
        ...
        for (const [x, y] of [[6, 18], [7, 18], [8, 18], [9, 18], [10, 18], [11, 18]])
            set(buf, x, y, sh);
    }
}
...
function drawHeadBack(buf, r) {
    ...
    for (const [x, y] of [[5, 3], [12, 3], [5, 13], [12, 13], [6, 14], [11, 14]])
        set(buf, x, y, sh);
    // crown sheen (rounded top catching the light) + subtle center part
    for (const [x, y] of [[7, 2], [8, 2], [9, 2], [10, 2], [7, 3], [8, 3], [9, 3]])
        set(buf, x, y, hi);
    for (let y = 4; y <= 11; y++)
        set(buf, 9, y, hi); // sheen down the crown
    for (let y = 4; y <= 12; y++)
        set(buf, 8, y, sh); // part line
    // nape + neck (skin)
...
function drawHeadBackBald(buf, r) {
    ...
    for (const [x, y] of [[7, 2], [8, 2], [9, 2], [8, 3], [9, 4], [9, 5]])
        set(buf, x, y, shi);
...
exports.AVATAR_RECIPES = RECIPES;
...
function compose(r) {
    CUR_W = exports.PORTRAIT_W;
    CUR_H = exports.PORTRAIT_H;
    const buf = new Uint8ClampedArray(exports.PORTRAIT_W * exports.PORTRAIT_H * 4);
...
function composeScene(r, phase, back) {
    CUR_W = exports.SCENE_W;
    CUR_H = exports.SCENE_H;
    const buf = new Uint8ClampedArray(exports.SCENE_W * exports.SCENE_H * 4);
 ...
 */
function registerAvatarOverride(name, r) {
    RECIPES[name] = r;
...
function paintPortrait(ctx, name, scale = 2) {
    ...
    stage.width = exports.PORTRAIT_W;
    stage.height = exports.PORTRAIT_H;
    const sctx = stage.getContext('2d');
    const img = sctx.createImageData(exports.PORTRAIT_W, exports.PORTRAIT_H);
    img.data.set(buf);
    sctx.putImageData(img, 0, 0);
    ctx.imageSmoothingEnabled = false;
    ctx.clearRect(0, 0, exports.PORTRAIT_W * scale, exports.PORTRAIT_H * scale);
    ctx.drawImage(stage, 0, 0, exports.PORTRAIT_W, exports.PORTRAIT_H, 0, 0, exports.PORTRAIT_W * scale, exports.PORTRAIT_H * scale);
}
