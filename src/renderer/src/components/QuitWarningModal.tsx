            ...
            {inClosingTime ? (
              <>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{
                      ...
                      {closing!.phase === 'complete'
                        ? t('quitWarning.phaseComplete')
                        : closing!.phase === 'timeout'
                          ? t('quitWarning.phaseTimeout')
                          : t('quitWarning.phaseProgress')}
                    </div>
                    <div style={{ fontSize: 15, lineHeight: '22px', color: 'var(--cth-ink-700)' }}>
                      {closing!.phase === 'complete'
                        ? t('quitWarning.bodyComplete')
                        : t('quitWarning.bodyProgress')}
                    </div>
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  {closing!.total > 0
                    ? <>{t('quitWarning.ackProgress', { acked: closing!.acked, total: closing!.total })}{closing!.acked >= closing!.total ? ` — ${t('quitWarning.waitingOrchestrator')}` : ''}</>
                    : t('quitWarning.noWorkers')}
                  {closing!.phase === 'timeout' && (
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{
                      ...
                      {ptyCount === 1
                        ? t('quitWarning.agentsRunningOne', { count: ptyCount })
                        : t('quitWarning.agentsRunningOther', { count: ptyCount })}
                    </div>
                    <div style={{ fontSize: 15, lineHeight: '22px', color: 'var(--cth-ink-700)' }}>
                      {ptyCount === 1
                        ? t('quitWarning.closeBodyOne')
                        : t('quitWarning.closeBodyOther', { count: ptyCount })}
                    </div>
                  </div>
                </div>
                ...
                {closing?.phase === 'error' && (
                  <div style={{
                    ...
                    {closing.error ?? t('quitWarning.errStart')}
                  </div>
                )}
                  ...
                  {onClosingTime && (
                    <PixelButton variant="primary" size="md" onClick={onClosingTime} disabled={busy}>
                      <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                        <Icon name="clock" /> {t('quitWarning.closingTimeBtn')}
                      </span>
                    </PixelButton>
                  )}
                  <PixelButton variant="destructive" size="md" onClick={confirm} disabled={busy}>
                    {busy ? t('quitWarning.killing') : (ptyCount === 1 ? t('quitWarning.killQuitOne') : t('quitWarning.killQuitOther'))}
                  </PixelButton>
                </div>
              </>
            )}
